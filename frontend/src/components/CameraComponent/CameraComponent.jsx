...
const CameraComponent = () => {
  ...
  useEffect(() => {
    const fetchWishlistProducts = async () => {
      ...
      try {
        const wishlistResponse = await fetch('http://wallandtone.com/api/wishlist', {
          method: 'GET',
          headers: {
        ...
        const productsResponse = await fetch('http://wallandtone.com/api/products', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    const fetchCartAndWishlist = async () => {
      ...
      try {
        const cartResponse = await fetch('http://wallandtone.com/api/cart', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
...
const handleProductSelect = (product) => {
  ...
  if (productImage) {
    productImage.src = `http://wallandtone.com/${product.mainImage}`;
  }
};
  ...
  const fetchProductData = async (productId) => {
    try {
      const response = await fetch(`http://wallandtone.com/api/products/${productId}`);
      if (!response.ok) {
...
const handleSizeSelect = (size) => {
  ...
  const handleAddToWishlist = async (product) => {
    ...
    try {
      await fetch('http://wallandtone.com/api/wishlist/add', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleRemoveFromWishlist = async (wishlistItem) => {
    try {
      await fetch(`http://wallandtone.com/api/wishlist/remove/${wishlistItem.productId._id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const fetchFrameTypes = async (productId) => {
    try {
      const response = await fetch(`http://wallandtone.com/api/products/${productId}/frame-types`);
      if (!response.ok) {
  ...
  const fetchSubFrameTypes = async (productId) => {
    try {
      const response = await fetch(`http://wallandtone.com/api/products/${productId}/sub-frame-types`);
      if (!response.ok) {
        if (response.status === 404) {
  ...
  const fetchSizes = async (productId) => {
    try {
      const response = await fetch(`http://wallandtone.com/api/products/${productId}/sizes`);
      const data = await response.json();
  ...
  const handleSubFrameTypeSelect = async (subFrameType) => {
    ...
    const formattedImageUrl = subFrameType.imageUrl?.startsWith("http")
      ...
      : `http://wallandtone.com/uploads/${subFrameType.imageUrl}`;
    ...
    try {
      console.log(`📤 Fetching image from API: http://wallandtone.com/api/products/${selectedProduct._id}/subframe-image/${subFrameType._id}`);
      
      const response = await fetch(
        `http://wallandtone.com/api/products/${selectedProduct._id}/subframe-image/${subFrameType._id}`
      );
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const response = await fetch('http://wallandtone.com/api/cart/add', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
...
const handleFrameTypeSelect = (frameType) => {
  ...
  const fetchSubFrameTypesByFrameType = async (frameTypeId) => {
    try {
      const response = await fetch(`http://wallandtone.com/api/sub-frame-types/${frameTypeId}`);
      if (!response.ok) {
        throw new Error(`Error fetching sub frame types: ${response.status} ${response.statusText}`);
      }
      const data = await response.json();
  ...
  {productDetails.subFrameType && (
    <img
    src={`http://wallandtone.com/uploads/${productDetails.subFrameType.imageUrl}`}
