...
const ProductDetails = () => {
  ...
  useEffect(() => {
    if (selectedFrameType?._id) {
      fetch(`http://localhost:5000/api/sub-frame-types/${selectedFrameType._id}`)
        .then(res => res.json())
  ...
  useEffect(() => {
    if (product?._id) {
      fetch(`http://localhost:5000/api/products/${product._id}/sizes`)
        .then(res => res.json())
  ...
  const handleFrameTypeSelect = async (frameType) => {
    try {
      const response = await fetch(`http://localhost:5000/api/frame-types/${frameType._id}`);
  ...
  const handleSubFrameTypeSelect = async (subFrameType) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/products/${product._id}/subframe-image/${subFrameType._id}`);
      if (!response.ok) {
    ...
    if (token) {
      try {
        const response = await fetch(
          `http://localhost:5000/api/wishlist/${inWishlist ? 'remove' : 'add'}`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  const handleUpdateQuantity = async (item, newQuantity) => {
    ...
    if (token) {
      try {
        const response = await fetch(
          `http://localhost:5000/api/cart/update/${item.productId._id}`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
  ...
  const handleRemoveFromCart = async (item) => {
    if (token) {
      try {
        const response = await fetch(
          `http://localhost:5000/api/cart/remove/${item.productId._id}/${item.frameType._id}/${item.subFrameType._id}/${item.size._id}`,
          {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
          }
