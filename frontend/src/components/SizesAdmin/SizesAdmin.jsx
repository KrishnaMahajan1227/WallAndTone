...
function SizesAdmin() {
  ...
  useEffect(() => {
    if (token) {
      axios.get('http://localhost:5000/api/admin/sizes/getsizes', {
        headers: {
  ...
  const handleSubmit = (event) => {
    ...
    if (mode === 'create') {
      axios.post('http://localhost:5000/api/admin/sizes/addsize', {
        width,
        height,
        price,
      }, {
      ...
      axios.put(`http://localhost:5000/api/admin/sizes/updatesize/${id}`, {
        width,
        height,
        price,
      }, {
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:5000/api/admin/sizes/deletesize/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
