...
const ProductManager = () => {
  ...
  const fetchSubFrameTypes = async () => {
    try {
      const response = await axios.get('http://backend.wallandtone.com/api/sub-frame-types');
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('http://backend.wallandtone.com/api/products');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = formData._id
        ? `http://backend.wallandtone.com/api/products/${formData._id}`
        : 'http://backend.wallandtone.com/api/products';
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      await axios.post('http://backend.wallandtone.com/api/products/excel', formDataToSend);
      fetchProducts();
  ...
  const handleAddSubframeImage = async (e) => {
    ...
    try {
      await axios.post(
        `http://backend.wallandtone.com/api/products/${selectedProduct._id}/subframe-images`,
        formDataToSend,
        {
          headers: {
...
const handleSubframeImageChange = (e) => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://backend.wallandtone.com/api/products/${id}`);
